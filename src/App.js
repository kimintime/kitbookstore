...
function App() {
  ...
  const fetchItems = () => {
    fetch('https://bookstore-a83e1-default-rtdb.europe-west1.firebasedatabase.app/books/.json')
  ...
  const deleteBook = (id) => {
    fetch(`https://bookstore-a83e1-default-rtdb.europe-west1.firebasedatabase.app/books/${id}.json`,
    {
      method: 'DELETE',
    })
    .then(response => fetchItems())
  ...
  return (
    <div className="App">
      ...
      <div className="ag-theme-material" style={{height: 400, width: 1075, margin: 'auto'}}>
        <AgGridReact rowData={books} animateRows={true} rowDragManaged={true}>
          <AgGridColumn sortable={true} filter={true} rowDrag={true} field='title' />
